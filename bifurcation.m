...
if nargin==0
    ...
    if ~isempty(vParamRg) || ~isempty(nNbPts) || ~isempty(vTimeRange) || ...
            ~isempty(nSelVar) || ~isempty(y0)
...
elseif strcmp(action,'Start')
    ...
    for k_var=vParamRg(1):(vParamRg(2)-vParamRg(1)+1)/nNbPts:vParamRg(2)
        %solver
        [~,Y]=m_dreTF(fcname,vTimeRange,y0,k_var);
        xutil=Y(1,nSelVar);
        ...
        if k_var==vParamRg(1)
            ...
            if (k_var < xlim(1)) || (xlim(2) < k_var) || (min(xutil) < ylim(1)) ...
                    || (ylim(2) < max(xutil))
                xmin=xlim(1);
                xmax=xlim(2);
                ymin=ylim(1);
